...
def _first_last_from_stop_times(zf: zipfile.ZipFile, keep_trip_ids: set, chunksize=300_000):
...
def fetch_routes() -> pd.DataFrame:
    print("Fetching Irish Citylink (TFI GTFS_All, streamed)…")
    z = _http_get(TFI_GTFS_ALL)
    return _build_df(z)
