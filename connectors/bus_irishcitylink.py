# connectors/bus_irishcitylink.py
...
# Transport for Ireland publishes combined GTFS_All.zip (includes Citylink).
TFI_GTFS_ALL = "https://www.transportforireland.ie/transitData/Data/GTFS_All.zip"
OPERATOR_NAME = "Irish Citylink"
AGENCY_MATCH  = ["citylink"]  # agency_name usually includes 'Citylink'
...
def _subset_by_agency(zf: zipfile.ZipFile, agency_terms):
    ...
    keep = pd.Series(False, index=agencies.index)
    for t in agency_terms:
        keep |= agencies["__n"].str.contains(t, na=False)
    return agencies.loc[keep, "agency_id"].astype(str).unique().tolist()

def _build_df(zbytes: bytes) -> pd.DataFrame:
    zf = zipfile.ZipFile(io.BytesIO(zbytes))
    keep_ids = _subset_by_agency(zf, AGENCY_MATCH)
    ...
    firsts = st.groupby("trip_id").first().reset_index()
    lasts  = st.groupby("trip_id").last().reset_index()
...
def fetch_routes() -> pd.DataFrame:
    print("Fetching Irish Citylink (TFI GTFS_All)…")
    z = _get_with_retries(TFI_GTFS_ALL)
    return _build_df(z)
