...
def _first_last_from_stop_times(zf: zipfile.ZipFile, keep_trip_ids: set, chunksize=500_000):
    ...
    with zf.open("stop_times.txt") as fh:
        ...
        for chunk in reader:
            ...
            first_parts.append(
                chunk.loc[idx_min, ["trip_id", "departure_time", "stop_id", "stop_sequence"]]
                .rename(columns={
            ...
            last_parts.append(
                chunk.loc[idx_max, ["trip_id", "arrival_time", "stop_id", "stop_sequence"]]
                .rename(columns={
...
def _build_df(zbytes: bytes) -> pd.DataFrame:
    ...
    if keep_agency_ids:
        routes = routes[
            (routes["agency_id"].astype(str).isin(keep_agency_ids)) | routes["agency_id"].isna()
        ]
